	...
	function kirim(){
		if (isset($_POST['submit'])){
			if ($this->input->post() && (strtolower($this->input->post('security_code')) == strtolower($this->session->userdata('mycaptcha')))) {
				...
				echo $this->session->set_flashdata('message', '<span class="flat-input"><div class="alert alert-success"><center>Pesan terkirim!, akan kami respon via email!</center></div></span>');
			}else{
				echo $this->session->set_flashdata('message', '<span class="flat-input"><div class="alert alert-danger"><center>Security Code yang anda masukkan salah!</center></div></span>');
			}
			redirect('hubungi');
		}
	}
